...
/** Убрать заказы из постоянного архива после отмены отправки */
export async function removeShippedOrdersFromArchive(orderIds: string[]): Promise<boolean> {
  const ids = orderIds.filter(Boolean);
  if (ids.length === 0) return true;

  try {
    const res = await fetchWithTimeout("/api/archive/unship", {
      method: "POST",
      headers: mutatingApiHeaders(),
